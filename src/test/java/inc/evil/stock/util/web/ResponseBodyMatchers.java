package inc.evil.stock.util.web;
...
public class ResponseBodyMatchers {
    ...
    public <T> ResultMatcher containsListAsJson(Object expectedObject, String fieldName) {
        return mvcResult -> {
            String json = mvcResult.getResponse().getContentAsString();
            JsonNode rootNode = objectMapper.readTree(json);
            JsonNode actualObject = rootNode.get("_embedded").get(fieldName);
            assertThat(actualObject).isEqualToComparingFieldByField(toJsonNode(expectedObject));
        };
    }
